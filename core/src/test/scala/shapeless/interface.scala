...
package rpcFramework {
  ...
  object ToClient {
    implicit def instance[T, Repr <: HList, F <: HList](implicit
      iface: Interface.Aux[T, Repr],
      f: FlattenParamLists.Aux[Repr, F],
      sf: SynthesizeFuncs[F]
    ): ToClient[T] = {
      new ToClient[T] {
        def apply(ep: Endpoint): T = {
          val funcs = sf.synthesizeFuncs(ep).funcs
          iface.from(f.nest(funcs))
        }
      }
    }
  }
  ...
  object ToEndpoint {
    ...
    implicit def instance[T, Repr <: HList, F <: HList](implicit
      iface: Interface.Aux[T, Repr],
      fpl: FlattenParamLists.Aux[Repr, F],
      th: ToHandlers[F]
    ): ToEndpoint[T] = {
      new ToEndpoint[T] {
...
package tracingFramework {
  ...
  object Tracer {
    ...
    implicit def traceInterface[T, Repr](implicit
      iface: LabelledInterface.Aux[T, Repr],
      tr: Tracer[Repr]
